...
export class ReportesMembresias implements OnInit {
    ...
    this.membersService.getMembers().subscribe({
      next: (data) => {
        ...
        if (data && Array.isArray(data) && data.length > 0) {
          ...
          this.members = data.map((m) => this.transformMemberData(m));
  ...
  transformMemberData(m: any): any {
    const transformed = { ...m };
    const productLower = (m.Producto || m.Tipo || '').toLowerCase();
    const isAbono =
      productLower.includes('abono') ||
      productLower.includes('combo') ||
      (m['Categoría'] || '').toLowerCase().includes('abono');

    // 1. Vendedor normalization
    const vendedor = (transformed.Vendedor || '').trim();
    if (!vendedor || vendedor.toLowerCase() === 'sin asignar') {
      transformed.Vendedor = 'SIN ASIGNAR';
    }

    // 2. Antigüedad normalization
    const antiguedad = (transformed['Antigüedad'] || '').toLowerCase().trim();
    if (antiguedad === 'antiguo' || antiguedad === 'renovacion' || antiguedad === 'renovación') {
      transformed['Antigüedad'] = 'Renovación';
    } else if (antiguedad === 'nuevo' || antiguedad === 'nueva') {
      transformed['Antigüedad'] = 'Nuevo';
    }

    if (isAbono) {
      // 3. Abono specific logic
      const originalCat = (m['Categoría'] || '').toLowerCase();

      // Determine Abono Product Type
      if (originalCat.includes('niño')) {
        transformed.Producto = 'Abono Niño';
      } else if (originalCat.includes('adulto mayor')) {
        transformed.Producto = 'Abono Adulto Mayor';
      } else {
        transformed.Producto = 'Abono';
      }

      // Determine Sector (Categoría)
      if (originalCat.includes('curva norte')) {
        transformed['Categoría'] = 'Curva Norte';
      } else if (originalCat.includes('curva sur')) {
        transformed['Categoría'] = 'Curva Sur';
      } else if (originalCat.includes('curva')) {
        transformed['Categoría'] = 'Curva';
      } else if (originalCat.includes('general')) {
        transformed['Categoría'] = 'Recta General';
      } else if (originalCat.includes('preferencia')) {
        transformed['Categoría'] = 'Preferencia';
      } else if (originalCat.includes('butaca')) {
        transformed['Categoría'] = 'Butaca';
      }

      // Información Adicional
      transformed['informacionAdicional'] = m['Categoría'] || '';
    } else {
      // 4. Membership (Sociedad) specific logic
      let cat = transformed['Categoría'] || '';

      // Remove "Socio " prefix
      if (cat.toLowerCase().startsWith('socio ')) {
        cat = cat.substring(6);
      }

      transformed['Categoría'] = cat.toUpperCase().trim();
      transformed['informacionAdicional'] = '';
    }
  ...
  populateFilterOptions() {
    ...
    if (!this.filterData.evento) {
      this.filterData.evento = this.uniqueEvents.includes(2025)
        ? 2025
        : this.uniqueEvents[0] || null;
    ...
    // Unique Antigüedad
    this.uniqueAntiguedad = [
      ...new Set(this.members.map((m) => m['Antigüedad']).filter((x) => x)),
    ].sort();
    ...
    const currentYearMembers = this.members.filter(
      (m) => String(m['Gestión']) === String(this.filterData.evento)
    );

    this.uniqueChannels = [
      ...new Set(currentYearMembers.map((m) => m.Vendedor).filter((x) => x)),
    ].sort();
  ...
  updateExecutiveList() {
    ...
    if (this.filterData.evento) {
      membersToConsider = this.members.filter(
        (m) => String(m['Gestión']) === String(this.filterData.evento)
      );
    }

    this.uniqueChannels = [
      ...new Set(membersToConsider.map((m) => m.Vendedor).filter((x) => x)),
    ].sort();
  ...
  updateCategoryList() {
    ...
    if (this.filterData.evento) {
      consideredMembers = this.members.filter(
        (m) => String(m['Gestión']) === String(this.filterData.evento)
      );
    }
    ...
    if (this.filterData.producto) {
      ...
      consideredMembers = consideredMembers.filter((m) => {
        ...
        } else if (selected === 'Sociedades') {
          // Sociedades
          return !productLower.includes('abono') && !productLower.includes('combo');
        } else {
          // Specific selection (e.g., Abono Niño)
          return m.Producto === selected;
        }
      });
    }
    ...
    this.uniqueTypes = [
      ...new Set(consideredMembers.map((m) => m['Categoría']).filter((x) => x)),
    ].sort();
  ...
  calculateStats() {
    ...
    const totalAmount = this.filteredMembers.reduce(
      (acc, curr) => acc + (curr['Total Ingreso'] || 0),
      0
    );

    // Calculate counts for Socios vs Abonos
    const abonosCount = this.filteredMembers.filter((m) =>
      (m.Producto || '').toLowerCase().includes('abono')
    ).length;
    const sociosCount = total - abonosCount;

    // Calculate by category
    const dorado = this.filteredMembers.filter((m) => m['Categoría'] === 'DORADO');
    const celeste = this.filteredMembers.filter((m) => m['Categoría'] === 'CELESTE');
    const platino = this.filteredMembers.filter((m) => m['Categoría'] === 'PLATINO');
    ...
    if (sortedVendors.length > 0) {
      ...
      this.stats[2].items = [{ label: 'N/A', value: '0 Bs' }];
    }

    // Card 4: Antigüedad (Nuevos vs Renovación)
    const nuevosCount = this.filteredMembers.filter((m) =>
      (m['Antigüedad'] || '').toLowerCase().includes('nuevo')
    ).length;
    const antiguosCount = this.filteredMembers.filter((m) =>
      (m['Antigüedad'] || '').toLowerCase().includes('renovación')
    ).length;

    this.stats[3].title = 'Antigüedad';
    this.stats[3].icon = 'clock'; 
    this.stats[3].color = 'primary';
    this.stats[3].items = [
      { label: 'Nuevos', value: nuevosCount.toLocaleString() },
      { label: 'Renovación', value: antiguosCount.toLocaleString() },
    ];
  }
  ...
  applyFilters() {
    this.filteredMembers = this.members.filter((m) => {
      ...
      if (
        ...
        !String(m.CI || '')
          .toLowerCase()
          .includes(this.filterData.ci.toLowerCase())
      ) {
        match = false;
      }
      ...
      if (this.filterData.producto) {
        ...
        if (selected === 'Combos') {
          ...
          // Specific Match (Abono Niño, Abono Adulto Mayor, etc.)
          if (m.Producto !== selected) match = false;
        }
      }
